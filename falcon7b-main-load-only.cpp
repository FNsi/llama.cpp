...
//        ctx_size += n_embd*ggml_type_sizef(GGML_TYPE_F32); // norm_b
        ...
        ctx_size += n_layer*(4672*n_embd*ggml_type_sizef(wtype)); // query_key_value
...
//        ctx_size += n_layer*(n_ff*ggml_type_sizef(GGML_TYPE_F32)); // w2_b
    ...
    // prepare memory for the weights
    {
        ...
        for (int i = 0; i < n_layer; ++i) {
            ...
            layer.query_key_value = ggml_new_tensor_2d(ctx, wtype, n_embd, 4672);
...
//                nelements = nelements - 1;
                ...
                if (n_dims == 1) {
                    ...
                    if (ggml_nelements(tensor) != (nelements)) {
                        fprintf(stderr, "%s: tensor '%s' has wrong size in model file: gg_elements %d, %d\n", __func__, name.data(), ggml_nelements(tensor), nelements);
                        return false;
                    }
                } else {
                    printf("%s[%d] = %d x %d %d\n", name.data(), i, ne[0]), ne[1], ne[2];
                    if (ggml_nelements(tensor) != nelements) {
                        fprintf(stderr, "%s: tensor '%s' has wrong size in model file: ggele %d, nparts %d, nele %d,\n", __func__, name.data(), ggml_nelements(tensor), n_parts, nelements);
                        return false;
                    }
                }
                ...
                if (n_dims == 1) {
                    if (tensor->ne[0] != ne[0] || tensor->ne[1] != ne[1]) {
                        fprintf(stderr,
                                "%s: tensor '%s' has wrong shape in model file: A got [%lld, %lld], expected [%d, %d]\n",
                    ...
                    if (split_type == 0) {
                        if (tensor->ne[0]/n_parts != ne[0] || tensor->ne[1] != ne[1]) {
                            fprintf(stderr,
                                    "%s: tensor '%s' has wrong shape in model file: B got [%lld, %lld], expected [%d, %d]\n",
                        ...
                        if (tensor->ne[0] != ne[0] || tensor->ne[1]/n_parts != ne[1]) {
                            fprintf(stderr,
                                    "%s: tensor '%s' has wrong shape in model file: C got [%lld, %lld], expected [%d, %d]\n",
                ...
                if (n_dims == 1 || n_parts == 1) {
                    if ((nelements*bpe)/ggml_blck_size(tensor->type) != ggml_nbytes(tensor)) {
                        fprintf(stderr, "%s: tensor '%s' has wrong size in model file A: got %zu, expected %zu\n",
                    ...
                    if ((nelements*bpe)/ggml_blck_size(tensor->type) != ggml_nbytes(tensor)/n_parts) {
                        fprintf(stderr, "%s: tensor '%s' has wrong size in model file B: got %zu, expected %zu\n",
